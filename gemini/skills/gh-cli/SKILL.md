...
# Using functions
gh pr list --template \
'{{range .}}{{tablerow .number .title .author.login}}{{"\n"}}{{end}}'

# Template with color
gh pr list --template \
'{{range .}}{{autocolor "green"}}{{.number}}{{autocolor "reset"}} {{.title}}{{"\n"}}{{end}}'

# Template with JSON
gh pr list --json number,title \
  --template '{{. | tojson}}'

# Template with time formatting
gh release list --template \
'{{range .}}{{.tagName}}: {{.createdAt | timeago}}{{"\n"}}{{end}}'
```
...
gh pr list --json url --jq '.[].url' | xargs -n 1 open

# Get list of contributors
gh issue list --json author --jq '.[].author.login' | sort | uniq

# Get list of open PRs by author
gh pr list --json author --jq '.[].author.login' | sort | uniq -c
```

#### Aliases with prompts

```bash
# Create alias with prompt
gh alias set create-bug \
  'issue create --title "Bug: $(gh prompt text -m "Enter bug title:")" --body "Steps: $(gh prompt editor)"'
```

#### JSON Schema

To get the JSON schema for a command, use the `--help --json` flag.

```bash
gh issue list --help --json
```
